...
TARGET_LENGTH = 896
TF_GAMMAS = torch.load("/om2/user/mabdel03/files/Classes/6.8611/Project/Baseline_Model/Batch_Training/Downloads/tf_gammas.pt", weights_only=True)
...
class DNADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if seq_len < self.sequence_length:
            raw_sequence = raw_sequence + 'N' * (self.sequence_length - seq_len)
...
model = ShallowEnformer.from_hparams(
    dim=1536,
    depth=11,
    heads=8,
...
# We extract stem, encoder, decoder from model.module
stem = model.module.stem
encoder = model.module.encoder
decoder = model.module.decoder

def train_autoencoder(model, stem, encoder, decoder, train_loader, val_loader, num_epochs=10):
    ...
    for epoch in range(num_epochs):
        ...
        encoder.train()
        decoder.train()
        stem.eval()  # stem used as a feature extractor

        total_loss = 0
        for batch_idx, batch in enumerate(train_loader):
            sequences = str_to_one_hot(batch['sequence']).to(device, non_blocking=True)

            # Forward pass through stem
            with torch.no_grad():
                stem_output = stem(sequences.transpose(1, 2))
            del sequences

            # Forward pass through autoencoder
            encoded = encoder(stem_output)
            ...
            # Clean up
            del stem_output, encoded, decoded

            if batch_idx % 10 == 0:
                avg_loss = total_loss / (batch_idx + 1)
                print(f"Epoch {epoch+1}/{num_epochs}, Batch {batch_idx}/{len(train_loader)}, Avg Loss: {avg_loss:.4f}")
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(val_loader):
                ...
                val_loss += reconstruction_loss(decoded, stem_output).item()
                num_val_batches += 1

                del sequences, stem_output, encoded, decoded
    ...
    torch.save(model.module.state_dict(), "/om2/user/mabdel03/files/Classes/6.8611/Project/New_Model/Autoencoder_Training/Models/Trained_AutoEnc_Params.pth")
    ...
    torch.save({
        ...
        'loss': avg_train_loss,
    }, "/om2/user/mabdel03/files/Classes/6.8611/Project/New_Model/Autoencoder_Training/Models/Trained_AutoEnc_State.pth")

train_autoencoder(model, stem, encoder, decoder, train_loader, val_loader, num_epochs=10)
